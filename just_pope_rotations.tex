...
\begin{quote}
\small
Notes: Dependent variable is QDANN corn yield in bushels per acre. Reference category: corn monoculture (C-C-C-C-C-C). Column (1) excludes weather and soil controls; column (2) includes full control vector. Field (tile\_field\_ID) and year fixed effects included throughout; standard errors two-way clustered at county and year. Stars indicate BH-FDR adjusted significance: $^{***}$FDR$<$0.01, $^{**}$FDR$<$0.05, $^{*}$FDR$<$0.10.
\end{quote}
...
% NOTE: significance here is read off the analytic two-way-clustered SEs in
% Table~\ref{tab:corn_jp_moments} col. 2. Those SEs do not reflect the generated-regressor
% problem in the cubed first-stage residual; the three-stage bootstrap
% (Section~\ref{subsec:bootstrap}, boot_skew draws) is the inference of record and some
% stars may weaken. The DIRECTION (predominantly negative, none significantly positive) is
% a property of the point estimates and is robust to the SE choice.
...
\begin{quote}
\small
Notes: RCI entered as a factor variable; reference category is RCI = 0 (corn monoculture). Column (1) excludes weather and soil controls; column (2) includes the full control vector. Field (tile\_field\_ID) and year fixed effects included in both columns; standard errors two-way clustered at county and year.
\end{quote}
...
negative and increase in magnitude with severity: a somewhat dry July is
associated with a 1.29 bushel-per-acre lower corn yield and a dry July with 1.88
bushels per acre, both relative to a normal July, though neither bin is
individually statistically significant. Second, the month-specific VPD terms are
not uniformly signed: July VPD carries a negative coefficient while June and
August VPD enter positively. This sign pattern is
...
\end{quote}
% TODO: the RCI x Dry-season interactions are non-monotone and individually imprecise
% (RCI3 x Dry = +5.36, RCI3.24 x Dry = -5.32, neither significant in the current table).
% Before drawing any buffering conclusion at extreme drought, use a joint test or a
% linear-in-RCI interaction rather than reading individual factor-level coefficients.
...
Second, our identification strategy relies on within-field variation in rotation sequences over time. The field fixed effects absorb any time-invariant field characteristic that could jointly drive rotation choice and yield, but identification then comes only from fields that change rotation pattern during the panel, and time-varying unobservables correlated with those switches remain a threat to identification.
...
% tables/corn_jp_skew.tex omitted: it duplicates the skewness column of
% Table~\ref{tab:corn_jp_moments} but on a different (unstandardised) scale despite an
% identical caption. Re-add only after reconciling the two so they report the same quantity.
