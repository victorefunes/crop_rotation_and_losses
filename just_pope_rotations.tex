...
The variance equation is stronger under the bootstrap than under analytic standard errors. All three features enter with signs consistent with the mean results: wider soybean spacing ($\text{soy\_gap}$, $-3.75$) and a more recent soybean placement ($\text{late\_soy}$, $-0.58$) are associated with \emph{lower} corn yield variance, while consecutive soybean years raise it ($\text{soy\_cons}$, $+6.88$). Because column (2) has a generated-regressor dependent variable, the inference of record is the $B=999$ field-level pairs cluster bootstrap that wraps both stages (Section~\ref{subsec:bootstrap}), the same design as the sequence-dummy variance stage in Table~\ref{tab:corn_jp_moments}. Under that bootstrap all three coefficients are significant---soybean gap and consecutive-soybean years at 1\%, recency at 5\%---whereas the analytic two-way-clustered standard errors leave the gap effect marginal ($p<0.10$) and the other two insignificant. The bootstrap intervals are several times tighter than the analytic ones, a gap that persists whether the analytic standard errors are clustered on county or on field, so it reflects the bootstrap's propagation of first-stage estimation error rather than the choice of clustering dimension. Field resampling captures the within-field serial correlation that dominates the panel but not residual within-county cross-field correlation, so we read wider soybean spacing as a robust variance-reducing feature, with recency and consecutiveness directionally consistent but resting more heavily on the bootstrap.
...
Second, the variance effects are less precisely estimated than the mean effects but point the same way. Wider soybean spacing is associated with lower corn yield variance; under the field-level pairs cluster bootstrap that is the appropriate inference for this generated-regressor stage (Section~\ref{subsec:bootstrap}) the spacing effect is significant at the 1\% level, with recency and consecutiveness also entering significantly and with signs that match the mean results. The analytic two-way-clustered standard errors are considerably wider and leave the spacing effect only marginal, so how hard one leans on the variance channel depends on which inference one adopts; we treat spacing as a robust risk-reducing feature and the other two as directionally consistent.
