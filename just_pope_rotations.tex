...
Soil characteristics come from gSSURGO, produced by the USDA Natural Resources Conservation Service at 30-meter resolution. We extract three field-level measures: the National Commodity Crop Productivity Index (NCCPI), root-zone available water storage (AWC), and soil organic carbon stock (SOC) in the 0-100-centimeter depth zone. All three are field-level and effectively time-invariant, and we do not identify them separately once county fixed effects and the monthly soil-moisture controls are included; we therefore drop them from the preferred specification and retain the monthly soil-moisture terms. Figure~\ref{fig:nccpi_corn_map} shows the spatial distribution of NCCPI across Illinois.
...
Table~\ref{tab:corn_rot_vpd} augments the sequence-mean specification with explicit growing-season weather terms, allowing rotation and drought effects to be read from a single model. Two patterns are clear. First, the drought bins are negative and increase in magnitude with severity: a somewhat dry July lowers corn yield by 1.29 bushels per acre and a dry July by 1.88 bushels per acre, both relative to a normal July and both statistically significant. Second, the month-specific VPD terms are not uniformly signed: July VPD carries a negative coefficient while June and August VPD enter positively. This sign pattern is consistent with corn agronomy: July spans the silking and pollination window, when high atmospheric moisture demand is most damaging, whereas moderate VPD earlier and later in the season need not depress yields.
...
Table~\ref{tab:corn_rci_vpd} interacts factor RCI with the July drought bins to ask whether more complex rotations buffer corn yields against dry conditions, the field-level analogue of the resilience mechanism documented experimentally by \citet{bowles2020}. The evidence is mixed and, as elsewhere in this paper, non-monotone in RCI. Under \emph{somewhat dry} conditions the interaction terms are predominantly positive, and significantly so at several RCI levels (for example, RCI = 2.00, 2.24, and 2.45), indicating that moderate rotational complexity partially offsets the roughly 2.7 bushel-per-acre penalty of a somewhat dry July. Under \emph{dry} conditions, however, the interactions change sign repeatedly across adjacent RCI levels and are individually imprecise, so the table does not support a clean claim that higher RCI buffers corn against severe drought. This pattern reinforces the paper's central message: RCI is too coarse and too non-monotone a summary of rotation structure to carry a stable interaction with weather risk, which is precisely the property that would be required for a linear RCI-based adjustment to crop insurance rates to be correctly specified.
