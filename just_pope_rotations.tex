...
% NOTE: significance in Table~\ref{tab:corn_jp_moments} already reflects the three-stage
% B=999 bootstrap of Section~\ref{subsec:bootstrap}. Verified 2026-09-02: col. 1 SEs equal
% sqrt(diag(jp_vcov_var.txt)) to 4 d.p. for all 28 sequences; col. 2 SEs equal
% sqrt(diag(jp_vcov_skew.txt)) rescaled by a single 1/s^3 factor (s ~ 12.4 bu/acre), so
% the standardized-skewness t-stats and stars are the bootstrap ones. Re-derived stars match
% the printed table exactly. Under this inference the three named sequences remain
% significant (C-C-C-S-S-C p=0.062, S-S-C-C-C-C p=0.038, C-S-C-S-C-C p<0.001), no sequence
% is significantly positive, and the C-C-C-C-S-C variance reduction stays marginal
% (z = -1.67, p = 0.095). Nothing weakens relative to the analytic SEs.
...
The variance equation for corn is considerably weaker. None of the three features moves corn yield variance at conventional significance: the soybean-gap coefficient is negative ($-3.75$) and marginally significant ($p<0.10$) on analytic two-way-clustered standard errors, consistent with wider spacing modestly compressing variance, while recency and consecutiveness are individually insignificant. We treat the variance results as directional rather than robust. The stage-2 standard errors in column (2) are analytic and do not correct for the generated-regressor problem; the sequence-dummy Just-Pope variance stage (Table~\ref{tab:corn_jp_moments}) carries the three-stage $B=999$ bootstrap of Section~\ref{subsec:bootstrap}, and applying the same bootstrap to this parsimonious variance equation would if anything widen the soybean-gap interval, so we do not lean on the marginal spacing effect.
...
$^{**}p<0.05$, $^{*}p<0.10$.
\end{quote}
% NOTE (units, 2026-09-02): VPD is in kPa. From tables/corn_summary_stats.tex, July vpd_7
% has mean ~1.1 kPa and SD ~0.65 kPa (raw, cross-field/cross-year). So the coefficients are
% bu/acre per kPa: vpd_7 = -18.91 => a one-raw-SD wetter-to-drier July move (~0.65 kPa) is
% about -12 bu/acre, but with field and year FE the identifying variation is a fraction of
% that raw SD, so the realized within-field effect is smaller. vpd_6 = +54.91/kPa is
% implausibly large for a structural June effect (even a 0.2 kPa move => +11 bu/acre) and
% most likely reflects collinearity within the June weather block (pr_6 quadratic, cGDD_6m,
% EDD_6, soil_6); vpd_8 = +11.68/kPa is smaller but similarly not cleanly identified. Do not
% quote per-kPa marginal effects for June or August; the text below interprets only the
% July sign.
% TODO (spec decision): the model contains BOTH continuous July VPD (vpd_7, GRIDMET monthly
% mean) and the July dry-season bins (Somewhat dry / Dry, from vpdmax_7). They are highly
% collinear. Either drop continuous vpd_7 from corn_rot_vpd / corn_rci_vpd, or keep it and
% state that vpd_7 carries the within-bin linear slope while the bins carry the threshold
% shift. Prose below now states the latter reading.

Table~\ref{tab:corn_rot_vpd} augments the sequence-mean specification with explicit growing-season weather terms, allowing rotation and drought effects to be read from a single model. VPD is measured in kPa, so the continuous \texttt{vpd\_6}, \texttt{vpd\_7}, and \texttt{vpd\_8} coefficients are bushels per acre per kPa; the model also includes the July dry-season bins, which are defined from a separate July-VPD-maximum series, so \texttt{vpd\_7} is best read as the within-normal-bin linear slope of July VPD and the bins as the threshold shifts at 1.9 and 2.1 kPa. Two patterns emerge. First, the drought bins are negative and increase in magnitude with severity: a somewhat dry July is associated with a 1.29 bushels-per-acre lower corn yield and a dry July with 1.88 bushels per acre, both relative to a normal July, though neither bin is individually statistically significant. Second, of the continuous monthly terms only July VPD is both correctly signed and interpretable: it enters negatively, consistent with July spanning the silking and pollination window when high atmospheric moisture demand is most damaging. The positive June and August VPD coefficients are large relative to the observed within-field VPD range (June in particular, at roughly 55 bu/acre per kPa) and are not separately identified from the rest of the monthly weather block; we do not interpret them structurally or quote per-kPa marginal effects from them.
...
\end{quote}
% NOTE (2026-09-02): the RCI x Dry-season interactions are non-monotone and individually
% imprecise (RCI3 x Dry = +5.36, RCI3.24 x Dry = -5.32, no stars). The prose below no longer
% draws a buffering conclusion at extreme drought -- it states only that the pattern is
% sign-flipping and imprecise. A formal joint Wald test on the nine RCI x Dry terms (or a
...
Table~\ref{tab:corn_rci_vpd} interacts factor RCI with the July drought bins to ask whether more complex rotations buffer corn yields against dry conditions, the field-level analogue of the resilience mechanism documented experimentally by \citet{bowles2020}. The evidence is mixed and, as elsewhere in this paper, non-monotone in RCI. Under \emph{somewhat dry} conditions the interaction terms are predominantly positive, though significant at only two RCI levels (RCI = 1.41 and 2.00), weakly indicating that moderate rotational complexity partially offsets the roughly 2.7 bushel-per-acre penalty of a somewhat dry July. Under \emph{dry} conditions, however, the interactions change sign repeatedly across adjacent RCI levels and are individually imprecise, so the table does not support a clean claim that higher RCI buffers corn against severe drought. This pattern reinforces the paper's central message: RCI is too coarse and too non-monotone a summary of rotation structure to carry a stable interaction with weather risk, which is precisely the property that would be required for a linear RCI-based adjustment to crop insurance rates to be correctly specified.
...
% Standardized stage-3 skewness is column (2) of Table~\ref{tab:corn_jp_moments} above.
% The separate tables/corn_jp_skew.tex (raw resid_cube scale, ~s^3 larger, mislabeled
% "standardized") was deleted 2026-09-02; write_jp_skew_tex() removed from
% code/write_jp_tables.R.
