...
%\begin{table}[H]
  ...
  %\begin{threeparttable}
    ...
    \begin{tablenotes}
      \footnotesize
      \item Notes: Means and standard deviations (in parentheses) for QDANN corn yields and key covariates, by rotation type.      
...
\end{equation}
%
 Where $Z_{it} = [\text{late\_soy}_{it},\ \text{soy\_cons}_{it},\ \text{soy\_gap}_{it},\n\text{soy}_{it}]$: the soy lag (periods since most recent soybean harvest), a consecutive-soy dummy, the minimum gap between soybean harvests, and the total number of soybean harvests in the window.
...
\begin{table}[H]
  \centering
  \caption{Effect of rotation patterns on yields}
  \label{tab:zvector}
  \begin{threeparttable}
    \input{tables/corn_jp_mean.tex}
    \begin{tablenotes}
      \small
      \item Notes: Dependent variable is corn yield in bushels per acre.
      QDANN mean and SCYM mean report stage-1 coefficients; QDANN variance
      and SCYM variance report stage-2 (OLS) coefficients on squared residuals.
      Controls include monthly precipitation (quadratic), GDD, EDD, VPD, soil
      moisture, and AWC for June--August. County and year fixed effects included.
      Standard errors two-way clustered at field and year level.
...
\begin{table}[H]
  \centering
  \caption{Rotation patterns and corn yields}
  \label{tab:corn_rot}
  \begin{threeparttable}
    \input{tables/corn_rot.tex}
    \begin{tablenotes}
      \small
      \item Notes: Dependent variable is QDANN corn yield in bushels per acre.
      Reference category: corn monoculture (C-C-C-C-C-C). Column (1) excludes
      weather and soil controls; column (2) includes full control vector.
      County and year fixed effects included throughout.
      Stars indicate BH-FDR adjusted significance: $^{***}$FDR$<$0.01,
      $^{**}$FDR$<$0.05, $^{*}$FDR$<$0.10.
    \end{tablenotes}
  \end{threeparttable}
\end{table}
...
\begin{table}[H]
  \centering
  \caption{Rotation patterns and soybean yields}
  \label{tab:soy_rot}
  \begin{threeparttable}
    \input{tables/soy_rot.tex}
    \begin{tablenotes}
      \small
      \item Notes: Dependent variable is QDANN soybean yield in bushels per
      acre. Reference category: soy monoculture (S-S-S-S-S-S). See
      Table~\ref{tab:corn_rot} notes for details on controls and clustering.
    \end{tablenotes}
  \end{threeparttable}
\end{table}
...
\begin{table}[H]
  \centering
  \caption{Just-Pope stage 2: corn yield conditional variance}
  \label{tab:corn_jp_var}
  \begin{threeparttable}
    \input{tables/corn_jp_var.tex}
    \begin{tablenotes}
      \small
      \item Notes: Dependent variable is squared first-stage residual from
      the corn yield mean regression. OLS estimates of stage-2 variance
      equation~\eqref{eq:stage2}. Reference category: corn monoculture.
      County and year fixed effects included. Two-way clustering at field
      and year level.
    \end{tablenotes}
  \end{threeparttable}
\end{table}
...
\begin{table}[H]
  \centering
  \caption{Rotation Complexity Index and corn yields}
  \label{tab:corn_rci}
  \begin{threeparttable}
    \input{tables/corn_rci.tex}
    \begin{tablenotes}
      \small
      \item Notes: RCI entered as a factor variable; reference category is
      RCI = 1.41 (lowest observed value in the corn-soy subsample). Column (1)
      includes all crop types; column (2) restricts to corn-soy fields only.
      County and year fixed effects included.
    \end{tablenotes}
  \end{threeparttable}
\end{table}
...
\begin{table}[H]
  \centering
  \caption{Rotation Complexity Index and soybean yields}
  \label{tab:soy_rci}
  \begin{threeparttable}
    \input{tables/soy_rci.tex}
    \begin{tablenotes}
      \small
      \item Notes: See Table~\ref{tab:corn_rci} notes. Dependent variable is
      QDANN soybean yield in bushels per acre.
    \end{tablenotes}
  \end{threeparttable}
\end{table}
