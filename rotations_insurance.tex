...
% TODO: see the detailed TODO at Section~\ref{subsec:rot_score_res} -- this 2.59 figure
% could not be independently reproduced from the current Table~\ref{tab:zvector}
% coefficients and needs to be regenerated by the scoring pipeline.
...
\begin{quote}
\small
Notes: RCI entered as a factor variable; reference category is RCI = 1.41 (lowest observed value in the corn-soy subsample). Column (1) excludes weather and soil controls; column (2) includes the full control vector. County and year fixed effects included in both columns.
\end{quote}
...
% TODO -- MAJOR, attempted and could not resolve by hand: the $+2.59$ figure for "the
% perfect alternating rotation" also appears verbatim in old/AAEA_presentation.tex, which
% used the presentation-era Z-vector coefficients (tables/yield_models.tex) rather than
% the current Table~\ref{tab:zvector} coefficients -- direct evidence this figure is a
% carryover that was never recomputed after the coefficients were revised (see the TODO
% under Table~\ref{tab:zvector} above for the magnitude/sign changes involved).
%
% An attempt was made to independently verify/correct this number by hand: taking
% S-C-S-C-S-C as the perfect alternating sequence (soy in years t-2, t-4, t-6; i.e.
% late\_soy $=-2$, soy\_cons $=0$, soy\_gap $=1$, nsoy $=3$, per the coding conventions
% stated in Section~\ref{subsec:rot_score} and used elsewhere in this paper) and applying
% the presentation-era coefficients, the result is $-8.31$, not the stated $+2.59$.
% Trying the alternative sequence C-S-C-S-C-S (soy in years t-1, t-3, t-5; late\_soy
% $=-1$) gives $-5.46$ under the same old coefficients -- also not $2.59$. Since neither
% candidate reproduces the anchor value even under the coefficients that supposedly
% produced it, the exact per-sequence Z-vector encoding used by the scoring pipeline does
% not match what can be inferred from the text alone, and this number cannot be reliably
% hand-corrected. The same is true for the $-11.18$/$+3.91$ range and the "14 distinct
% values" count, both of which depend on Z-vector values for all 64 sequences that are not
% available outside the pipeline. All of these figures need to be regenerated directly
% from the current Table~\ref{tab:zvector} coefficients by the scoring code itself, not
% estimated by hand from this paper's text.
...
Third, the RCI does not move monotonically with corn yields: coefficients rise from RCI $=1.73$ to RCI $=2.24$, dip back down at RCI $=2$, and continue rising through RCI $=2.65$ rather than leveling off. This non-monotonicity, not a flattening beyond any particular threshold, is what a linear premium adjustment based on RCI would miss.
 
Fourth, we develop a rotation score, constructed from the same four structural sequence features, that maps the 64 binary rotation sequences onto a compact scalar suitable as a rating input.
% TODO: this paragraph previously claimed the score maps onto "14 distinct values" and is
% "linearly related to yield outcomes across most of its support." Both claims were
% originally derived using the Z-vector coefficients in Table~\ref{tab:zvector}, which
% have since been corrected (see the TODO in Section~\ref{subsec:rot_score_res}) -- the
% count of distinct score values and the shape of the score-yield relationship should be
% re-derived from the current coefficients before being restated here, since neither is
...
To make this evidence usable for rating purposes, we construct a rotation score from four structural features of the rotation sequence-the recency of the last soybean harvest, whether soybean years were consecutive, the minimum gap between soybean harvests, and the total count of soybean years-that collapses the 64 possible six-year corn-soy sequences into a compact, field-computable scalar. Because it is built entirely from Cropland Data Layer histories that USDA already maintains, the score could in principle be attached to any insured field at no additional data cost, offering a concrete alternative to RCI-based rating reforms.
...
Figure~\ref{fig:score_yield} reports predicted corn yields as a function of the rotation score. Score values range from approximately $-11.18$ (most soybean-heavy sequences with consecutive soybean years) to $+3.91$ (sequences with a very recent and optimally spaced soybean placement), with corn monoculture normalized to zero and the perfect alternating rotation scoring $+2.59$. The score-yield relationship is strongly positive and approximately linear across the upper portion of the score distribution (scores above $-2$). The score collapses 64 binary rotation sequences into 14 distinct score values, with variance effects showing a monotone negative relationship with score across most of the distribution.
% TODO: verify -- these score-range figures ($-11.18$ to $+3.91$, perfect rotation $=+2.59$)
% were computed from the four Z-vector coefficients in Table~\ref{tab:zvector}, per the
% formula in Figure~\ref{fig:score_yield}'s notes. Those coefficients were substantially
% revised earlier in this section (the previous version of this text described a
% QDANN/SCYM split with much larger magnitudes -- e.g. late_soy $\approx 2.85$ -- that did
% not match the actual table; the corrected values are roughly 5-10x smaller, e.g.
% late\_soy $= 0.41$). This range was not recomputed after that correction and should be
% re-derived from the current Table~\ref{tab:zvector} coefficients before being cited
% elsewhere (including in the Conclusion).
