...
Two coding choices deserve note. First, winter wheat enters as a rotation-history covariate rather than as a modeled outcome: corn and soybean remain the outcome crops, and wheat appears only in the six-year predecessor window. Second, we initially considered extending the rotation set to alfalfa and to the winter-wheat/soybean double crop, both of which are agronomically distinct (alfalfa is a perennial nitrogen-fixing forage). In the Illinois sample, each accounts for well under 1\% of field-years and does not appear in any of the 49 sequences that clear the 99\% support threshold used for the main sequence-dummy, RCI, and Z-vector tables, so we restrict those analyses to corn, soybean, and winter wheat. The LASSO analysis in Section~\ref{subsec:lasso_results}, which considers a substantially larger unrestricted candidate set, does recover one alfalfa-containing sequence for soybean with a large and precisely estimated coefficient, indicating alfalfa is not entirely unidentifiable in this sample, only too rare to survive a frequency-based restriction. We discuss this further in Section~\ref{subsec:limitations}.
...
% the already-correct pattern for tables/summary_stats.tex earlier in the file, which
% avoids this by commenting out its outer wrapper.
