...
Figure~\ref{fig:corn_var_plot} and the variance results in Table~\ref{tab:corn_jp_var} show the corresponding effects on the conditional variance of corn yields. As with the mean effects, Figure~\ref{fig:corn_var_plot} separates the corn--soybean and corn--soybean--wheat sequences into panels, and the wheat panel's variance coefficients carry very wide confidence intervals on thin support. Sequences that raise mean yields also tend to reduce yield variance, implying joint mean-variance dominance over monoculture. Sequences with a very recent soybean placement (year $t-1$) tend to show negative variance coefficients; $C-C-C-C-S-C$, for example, carries a statistically significant variance reduction of 11.3 units at the 5 percent level (Table~\ref{tab:corn_jp_var}). Figure~\ref{fig:corn_jp_plot} summarizes the joint mean-variance results in a single scatter plot.
...
Table~\ref{tab:soy_rot} and Figure~\ref{fig:soy_rot_plot} report sequence-level effects on soybean yields. The pattern is qualitatively similar to the corn case but with smaller magnitudes-the highest-performing sequences carry premiums of 1.43 to 1.68 bushels per acre relative to soybean monoculture (Table~\ref{tab:soy_rot}, column 2), compared to premiums of up to 4.61 bushels per acre for corn. This asymmetry is expected: the nitrogen-fixing benefit flows primarily from soybean to subsequent corn, not in the reverse direction. Variance effects for soybean (Figure~\ref{fig:soy_var_plot}) are smaller in magnitude than the corn variance effects and are statistically significant mainly for sequences with recent corn placements. As in the corn analysis, the soybean sequence set spans corn, soybean, and winter wheat (40 sequences, 9 of them containing wheat), and Figures~\ref{fig:soy_rot_plot} and~\ref{fig:soy_var_plot} separate the corn--soybean and corn--soybean--wheat sequences into panels; the wheat-containing sequences are again the minority and rest on thinner support with correspondingly wider intervals.
