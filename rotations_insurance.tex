...
Table~\ref{tab:corn_jp_skew} extends the mean-variance results to the third moment, reporting the effect of each rotation sequence on the conditional skewness of corn yields, estimated as a third-stage regression of the cubed first-stage residual on the same specification (following \citet{antle1983}). The sign convention is the reverse of the variance stage: a \emph{positive} skewness coefficient indicates that a sequence shifts probability mass toward the right tail relative to corn monoculture, thinning the left tail and \emph{reducing} downside risk, whereas a negative coefficient signals heavier left-tail exposure. This is the margin that maps most directly onto insurance loss cost, since indemnities are paid on low-yield realizations rather than on variance per se; a sequence can lower variance while leaving---or worsening---the probability of a bad harvest. Read alongside the mean and variance columns, the skewness estimates identify which sequences are unambiguously preferred to monoculture in the sense of \citet{difalco2009}: higher expected yield, lower variance, and no increase in downside risk.
% TODO: fill in the empirical specifics from tables/corn_jp_skew.tex once available in this
% working copy -- (i) which rotation sequences carry statistically significant positive
% (downside-risk-reducing) vs negative skewness coefficients, and (ii) whether the sequences
% flagged as mean-increasing / variance-reducing also clear the skewness screen. Confirm the
% table \label is tab:corn_jp_skew (assumed here from the corn_jp_mean / corn_jp_var naming
% convention). Inference on the cubed-residual coefficients should come from the three-stage
% bootstrap (Section~\ref{subsec:bootstrap}), not the analytic clustered SEs, since the
% stage-3 regressor is a generated variable built from stage-1 fitted values.
