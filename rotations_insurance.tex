...
Of the 102 candidate corn sequences considered (the unrestricted sample, prior to any 99\%-frequency cut), 28 have nonzero post-selection coefficients; for soybean, 24 sequences are selected. Full post-selection refit coefficients are reported in Table~\ref{tab:corn_lasso} and Table~\ref{tab:soy_lasso}, discussed in Section~\ref{subsec:lasso_results}.
% TODO: the soybean candidate-pool size (analogous to corn's 102) was not in the console
% output shared and should be added here once available. The cv.glmnet cross-check
% (selected-sequence overlap with the rlasso plug-in penalty) was also not reported and
% should be added as a robustness footnote once run.
...
Of the 102 candidate corn sequences in the unrestricted sample, 28 survive LASSO selection with a nonzero post-selection coefficient (Table~\ref{tab:corn_lasso}); for soybean, 24 sequences are selected (Table~\ref{tab:soy_lasso}). % TODO: state the soybean candidate-pool size once available, and the resulting selection rate, alongside corn's 28/102 (~27\%).
