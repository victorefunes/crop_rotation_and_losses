...
score provides a tractable rating instrument that could condition premiums on rotation history.
 
The paper proceeds as follows. Section~\ref{sec:litreview} reviews the relevant literature on rotation effects and crop insurance rating. Section~\ref{sec:data} describes our data sources and the construction of rotation sequences. Section~\ref{sec:methods} presents the econometric framework. Section~\ref{sec:results} reports results. Section~\ref{sec:conclusions} discusses implications for insurance rating and concludes.
...
\begin{table}[H]
  ...
  \begin{threeparttable}
    \input{tables/summary_stats_by_rotation.tex}
    \begin{tablenotes}
      \small
      \item Notes: Means and standard deviations (in parentheses) for QDANN corn yields and key covariates, by rotation type. Corn monoculture = C-C-C-C-C-C; Perfect rotation = alternating C-S sequence; Transitioning = sequences with RCI between 1.41 and 2.00.
    \end{tablenotes}
  \end{threeparttable}
\end{table}
...
\section{Rotation Patterns and Coefficient Tables}
\label{app:full_tables}
 
\begin{table}[H]
  \centering
  \caption{Just-Pope factor RCI: corn yield moments}
  \label{tab:corn_rci_jp}
  \begin{threeparttable}
    \input{tables/corn_rci_jp.tex}
    \begin{tablenotes}
      \small
      \item Notes: Column (1) reports stage-1 mean coefficients; column (2)
      reports stage-2 OLS variance coefficients. Reference: RCI = 1.41.
      Corn-soy fields only. Two-way clustering at field and year level.
    \end{tablenotes}
  \end{threeparttable}
\end{table}
...
\begin{table}[H]
  \centering
  \caption{Just-Pope factor RCI: soybean yield moments}
  \label{tab:soy_rci_jp}
  \begin{threeparttable}
    \input{tables/soy_rci_jp.tex}
    \begin{tablenotes}
      \small
      \item Notes: See Table~\ref{tab:corn_rci_jp} notes. Dependent variable
      is soybean yield.
    \end{tablenotes}
  \end{threeparttable}
\end{table}
...
\begin{table}[H]
  ...
  \begin{threeparttable}
    ...
    \begin{tablenotes}
      \small
      \item Notes: VPD bins defined as: normal ($<$1.9 kPa), somewhat dry
      (1.9--2.1 kPa), dry ($>$2.1 kPa). All bins measured in July (peak
      corn stress period). Reference: normal VPD, corn monoculture.
    \end{tablenotes}
  \end{threeparttable}
\end{table}
...
\begin{table}[H]
  \centering
  \caption{Effect of weather (VPD) and rotation sequences on soybean yields}
  \label{tab:soy_rot_vpd}
  \begin{threeparttable}
    \input{tables/soy_rot_vpd.tex}
    \begin{tablenotes}
      \small
      \item Notes: See Table~\ref{tab:corn_rot_vpd} notes. Dependent variable
      is soybean yield.
    \end{tablenotes}
  \end{threeparttable}
\end{table}
...
\begin{figure}[H]
  ...
  \caption{QDANN soybean yields across Illinois, 2016.}
  \label{fig:soy_yield_map}
  \floatfoot{\footnotesize Notes: Spatial distribution of QDANN field-level
  soybean yield estimates for Illinois in 2016. Source: \citet{maQDANN2024}.}
\end{figure}
 
\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\textwidth]{figures/nccpi_soy_map}
  \caption{National Commodity Crop Productivity Index (soybeans) across Illinois.}
  \label{fig:nccpi_soy_map}
  \floatfoot{\footnotesize Notes: NCCPI soybean values at 30-meter resolution.
