...
Two coding choices deserve note. First, winter wheat enters as a rotation-history covariate rather than as a modeled outcome: corn and soybean remain the outcome crops, and wheat appears only in the six-year predecessor window. Second, we initially considered extending the rotation set to alfalfa and to the winter-wheat/soybean double crop, both of which are agronomically distinct (alfalfa is a perennial nitrogen-fixing forage). In the Illinois sample, however, each accounts for well under one percent of field-years and does not appear in any sequence clearing the 99\% support threshold, so neither can be separately identified. We therefore restrict attention to corn, soybean, and winter wheat, and note the omission of perennial forages in Section~\ref{subsec:limitations}.
...
\medskip\noindent\textbf{Principal components analysis of rotation features}. To motivate the parsimonious Z-vector parameterization, we conduct a principal components analysis (PCA) of the rotation feature space across the 49 retained six-year sequences, using six structural features that generalize the corn--soybean summaries to the three-crop set: legume-year count, small-grain (wheat) count, sequence diversity, a non-monoculture measure, and two legume-spacing terms. Figure~\ref{fig:rot_pca_plot} shows the biplot of the first two principal components. PC1 explains 47.9 percent of the total variance and PC2 explains 39.0 percent, so together they account for roughly 87 percent. Unlike the corn--soybean-only case, in which a single component dominated, the variance now splits across two comparable axes: PC1 separates soybean-legume intensity and spacing (loading negatively) from winter-wheat presence (loading positively), while PC2 is driven by wheat count, sequence diversity, and non-monoculture. This two-dimensional structure reflects that the timing of legume placement and the inclusion of winter wheat are largely orthogonal features of a rotation, so a single scalar index captures the feature space less completely than in the binary corn--soybean case.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.75\textwidth]{figures/rot_pca_plot}
  \caption{PCA biplot of rotation features: corn, soybean, and winter wheat.}
  \label{fig:rot_pca_plot}
  \floatfoot{\footnotesize Notes: Biplot of the first two principal components across the 49 six-year rotation sequences covering approximately 99\% of field-years. PC1 explains 47.9\% of the total variance and PC2 explains 39.0\%. Red arrows show loadings of the six rotation summary features: $n\_\text{legume}$, $n\_\text{grain}$, diversity, $\text{no\_mono}$, $\text{free\_leg}$, and $\text{tight\_leg}$.}
\end{figure}

The same PCA applied to the soybean-terminal sequences (Figure~\ref{fig:rot_pca_plot_soy}) shows a similar two-dimensional structure, with PC1 explaining 62.8 percent of the variance and PC2 20.4 percent.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.75\textwidth]{figures/rot_pca_plot_soy}
  \caption{PCA biplot of rotation features for the soybean sample.}
  \label{fig:rot_pca_plot_soy}
  \floatfoot{\footnotesize Notes: Biplot of the first two principal components across the six-year sequences ending in soybean (the soybean estimation sample). PC1 explains 62.8\% of the total variance and PC2 20.4\%. Loadings are the same six rotation summary features as in Figure~\ref{fig:rot_pca_plot}.}
\end{figure}
...
Third, our analysis covers corn, soybean, and winter-wheat rotations in Illinois. Winter wheat enters through a minority of sequences and on thin support, so its estimates should be read as suggestive; perennial forages such as alfalfa, together with winter-wheat/soybean double crops, were too rare in this sample---each well under one percent of field-years, and absent from every sequence clearing the 99\% support threshold---to identify separately, and we leave them to settings where they are more common. Generalizability to other Corn Belt states and to different climate and soil regimes is an open question. Finally, rotation decisions respond endogenously to prices and the proximity of grain market infrastructure such as ethanol refineries \citep{Pates2025,cheu2025}. A full welfare analysis of rotation-conditioned rates would require modeling the joint distribution of rotation choice and yield outcomes under the counterfactual rate schedule, which we leave for future work.
