...
Our main findings are three. First, a handful of rotation sequences significantly outperform corn monoculture in terms of higher mean yields, but these sequences share identifiable structural features-specifically, a soybean harvest within the past one to two years, no consecutive soybean years, and a moderate number of total soybean harvests in the six-year window. Second, we construct a rotation score from these structural features that maps the observed six-year rotation sequences, drawn from corn, soybean, and winter-wheat histories, onto a scalar index. The score ranges from approximately $-144$ to $+229$ relative to corn monoculture, normalized to zero. % TODO: this range reflects the current (wheat-inclusive) score\_yield.png figure; see the detailed TODO at Section~\ref{subsec:rot_score_res} regarding the out-of-sample use of corn-soy-fitted Z-vector coefficients for wheat-containing sequences, and confirm the exact score for the perfect alternating rotation from the scoring pipeline rather than this text. Third, the Rotational Complexity Index (RCI)-the most natural scalar summary of rotation diversity-has a nonlinear and non-monotone relationship with both yield moments, which implies that linear RCI-based rating adjustments would be misspecified.
...
Table~\ref{tab:zvector} reports the parsimonious parameterization of rotation effects in terms of the four structural features. For corn yields, the most recent soybean placement variable ($\text{late\_soy}$) carries a positive and significant mean effect of 0.41 bushels per acre ($p<0.01$), and the soy gap variable carries a smaller positive effect of 0.16 bushels per acre, significant at the 10 percent level. The number of soybean harvests variable carries a positive and significant effect of 0.72 bushels per acre ($p<0.01$): controlling for recency and spacing, more soybean years in the window raise corn yields. The consecutive soybean years dummy carries a negative but statistically insignificant mean effect ($-0.30$ bushels per acre).

The variance equation for corn tells a different story. Soy gap and number of consecutive soybean years are the two variables with a significant effect on corn yield variance: a larger gap between soybean years reduces variance by 2.14 units ($p<0.01$), while consecutive soybean years raise variance by 9.05 units, significant at the 10 percent level. Neither the recency nor the frequency of soybean placement has a significant effect on corn yield variance in this specification. For soybean yields (columns 3-4), the number of soybean harvests is the only variable with a significant mean effect ($-0.30$ bushels per acre, $p<0.01$), while both soy gap and consecutive soybean years raise soybean yield variance significantly (0.16 and 0.86 units, respectively). The consecutive-soybean-years variance effect is therefore the one structural feature with a significant, same-signed (variance-increasing) effect on both crops.
...
Figure~\ref{fig:score_yield} reports predicted corn yields as a function of the rotation score, now computed over the 49-sequence corn--soybean--wheat sample. Score values span a much wider range than in the corn-soy-only version, running from approximately $-144$ to $+229$, with corn monoculture normalized to zero. The score-yield relationship is positive overall but, consistent with the sequence-level and RCI results above, noisy in the middle of the distribution rather than cleanly monotone; variance effects show no clean monotone relationship with score across the distribution.
% TODO -- IMPORTANT, flagged rather than silently corrected: the caption states the score
...
% TODO: the exact number of distinct score values and the score for a specific reference
% sequence (e.g. the old "+2.59 for the perfect alternating rotation" claim) cannot be
% read reliably off the plot and should be pulled directly from the scoring code's output
% table rather than estimated from the figure.
...
First, the effect of crop rotation on corn yields depends critically on the timing and spacing of soybean placements within the six-year rotation window, not on the total amount of soybean in the rotation. A recent soybean harvest raises corn yields by approximately 0.41 bushels per acre ($p<0.01$), and a larger gap between soybean years raises corn yields by approximately 0.16 bushels per acre ($p<0.10$); the number of soybean harvests in the window raises corn yields by approximately 0.72 bushels per acre ($p<0.01$). Consecutive soybean years carry a negative but statistically insignificant mean effect on corn yields.

Second, the mean and variance effects of these structural features do not always move together. A larger gap between soybean years both raises mean corn yields and lowers corn yield variance (by 2.14 units, $p<0.01$), so wider spacing is unambiguously better on both moments. Consecutive soybean years, by contrast, have no significant mean effect but a significant, variance-\textit{increasing} effect on both crops (9.05 units for corn at the 10 percent level; 0.86 units for soybean at the 1 percent level): the risk cost of bunching soybean years shows up in variance even where it does not show up in the mean.
...
Fourth, we develop a rotation score, constructed from the same four structural sequence features, that maps the observed six-year rotation sequences onto a compact scalar suitable as a rating input.
% TODO: this paragraph previously claimed the score maps onto "14 distinct values" and is
% "linearly related to yield outcomes across most of its support." The regenerated
% score\_yield.png figure (Section~\ref{subsec:rot_score_res}) shows a wider range
% (approximately -144 to +229) and a noisier, less clearly linear relationship than the
% old claim -- both the exact count of distinct values and the shape claim should still be
% re-derived from the scoring pipeline's output table rather than read off the figure, since neither is
% guaranteed to survive a roughly 5-10x change in coefficient magnitude.
 
\subsection{\label{subsec:implications}Implications for Crop Insurance Rating}
 
Our findings have direct implications for how rotation information could be incorporated into federal crop insurance rates. The rotation score provides a tractable rating instrument: a premium discount for fields with high rotation scores and a surcharge for fields with consecutive soybean years could be implemented within the existing APH framework by conditioning the base premium rate on the CDL-derived score, which is publicly available at the field level at no marginal cost to the insurer. Holding APH constant, a field with consecutive soybean years faces approximately 9.05 additional units of conditional corn yield variance (significant at the 10 percent level) and 0.86 additional units of conditional soybean yield variance (significant at the 1 percent level); translating these variance-unit effects into expected-indemnity dollar terms under Revenue Protection is a natural next step but has not been done here.
