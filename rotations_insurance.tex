...
% inconsistency. SCYM has been removed from the paper, so the SCYM sample size is no longer
% reported (prior corn-soy-only figure was 1,548,319 QDANN; the 1,411,846 SCYM figure is dropped).
...
We construct the estimation sample by merging the QDANN yield raster with CDL rotation sequences, GRIDMET and TerraClimate weather covariates, and gSSURGO soil variables at the 30-meter pixel level. The final sample comprises 1,548,319 field-year observations.
...
% RESOLVED: SCYM has been removed from the paper entirely (per author instruction, SCYM is
% not used). QDANN is now the sole yield estimator throughout -- intro, Section
% ~\ref{subsec:yield}, the sample-construction sentence, the abstract, and the Conclusion
% are consistent on this. The "1,411,846 (SCYM)" figure has been dropped.
