...
\textbf{Principal components analysis of rotation features}. To motivate the choice of four structural features as a sufficient summary of the 64 binary rotation sequences, we conduct a principal components analysis (PCA) of the RCI-level rotation feature space. Figure \ref{fig:rot_pca_plot} shows a biplot of the first two principal components of the rotation feature matrix, with each point representing one of the 18 distinct RCI values observed in the Illinois corn-soy sample and the red arrows representing loadings of four candidate summary variables: $n\_\text{soy}$ (number of soybean harvests), $\text{no\_consec}$ (absence of consecutive soybean years), $\text{tight\_soy}$ (soybean placements clustered close together), and $\text{free\_soy}$ (soybean placements well-spaced across the window).

\begin{figure}[H]
    \centering
    \includegraphics[scale = 0.55]{figures/rot_pca_plot.png}
    \caption{First two principal components of feature matrix}
    \label{fig:rot_pca_plot}
    \end{figure}

The first principal component (PC1) explains 83.3 percent of the total variance in the rotation feature space, and the second (PC2) explains an additional 9.6 percent, so that together the first two components account for approximately 93 percent of all variation across RCI levels. This near-complete dimensionality reduction confirms that the 18 distinct RCI values in our sample are well characterized by a very small number of underlying dimensions, thereby validating the parsimonious Z-vector parameterization relative to the full sequence-indicator model.

The biplot loadings reveal the structure of these dimensions. PC1 is dominated by $\text{tight\_soy}$ and $\text{free\_soy}$, which load in similar directions along the horizontal axis — both are primarily measures of soybean frequency and spacing that increase with the number of soybean years in the window. $n\_\text{soy}$ loads strongly in the upper-right quadrant, nearly orthogonal to PC1, indicating that the total number of soybean harvests captures a dimension of variation largely independent of how those harvests are spaced. $\text{no\_consec}$ loads toward the upper left, reflecting that the absence of consecutive soybean years is associated with lower total soybean frequency (fewer soybean years means fewer opportunities for consecutive placements). The near-orthogonality of $n\_\text{soy}$ and $\text{no\_consec}$ in the biplot confirms that these two variables are capturing distinct dimensions of rotation structure that would be conflated in a scalar index such as the RCI.

The PCA results motivate our Z-vector parameterization in two ways. First, the high proportion of variance explained by the first two components implies that four well-chosen variables suffice to characterize the rotation feature space, without information loss relative to the full 64-sequence indicator model. Second, the biplot loadings directly suggest which variables belong in the Z-vector: variables that span both principal component axes (covering the dominant sources of variation) rather than variables that are collinear along a single axis.
...
\begin{equation}
    \text{score}_{it} = \hat{\tau}_1 \cdot \text{late\_soy}_{it} + \hat{\tau}_2 \cdot \text{soy\_cons}_{it} + \hat{\tau}_3 \cdot \text{soy\_gap}_{it} + \hat{\tau}_4 \cdot n\text{soy}_{it}
\end{equation}

where $\hat{\tau}_1, \ldots, \hat{\tau}_4$ are the QDANN stage-1 estimates from Table 1. The score summarizes, in units of predicted yield deviation from monoculture, the information contained in the six-year rotation history relevant to the conditional mean of corn yields. By construction, corn monoculture — which has no soybean years in the window — scores zero. The perfect alternating corn-soybean rotation scores approximately 2.59 bushels per acre above monoculture under the QDANN specification.
...
Table 2 and Figure 1 report the estimated effects of each of the 31 non-monoculture six-year corn-soy sequences on corn yield mean, relative to corn monoculture. The full set of coefficient estimates reveals a striking pattern: rotation effects are heterogeneous across sequences and depend critically on the recent history of soybean placement rather than on the total number of soybean years in the window.

Several sequences significantly outperform corn monoculture in mean corn yield. The highest-performing sequences in the QDANN specification — those with estimated mean yield premiums exceeding 4 bushels per acre — share a common structural feature: a soybean harvest in year $t-1$ or $t-2$ (immediately or one year prior to the outcome corn year), regardless of what was planted in years $t-3$ through $t-6$. For example, $S-S-C-S-S-C$ (soybeans in years 6, 5, 3, and 2, corn in years 4 and 1) carries an estimated premium of 4.61 bushels per acre relative to monoculture (Table 2, column 2), and $S-C-S-C-S-C$ (the near-perfect alternating rotation with an additional soy lag) carries 4.12 bushels per acre. Both SCYM and QDANN yield consistent rankings at the top of the sequence distribution.


    \newpage

\input{tables/corn_stage1}
